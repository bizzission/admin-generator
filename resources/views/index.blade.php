...
{{'@'}}section('title', trans('{{ $viewNamespace }}admin.{{ $modelLangFormat }}.actions.index'))

{{'@'}}section('body')

<{{ $modelJSName }}-listing :data="{{'{{'}} $data->toJson() }}" :url="'{{'{{'}} url('{{ $routePrefix }}admin/{{ $resource }}') }}'" @if($containsPublishedAtColumn) :trans="{{'{{'}} json_encode(trans('brackets/admin-ui::admin.dialogs')) }}" @endif inline-template>

    <div class="row">
        <div class="col">
            <div class="card">
                <div class="card-header">
                    <i class="fa fa-align-justify"></i> {{'{{'}} trans('{{ $viewNamespace }}admin.{{ $modelLangFormat }}.actions.index') }}
                    @if($export)
                    <a class="btn btn-primary btn-sm pull-right m-b-0 ml-2" href="{{'{{'}} url('{{ $routePrefix }}admin/{{ $resource }}/export') }}" role="button"><i class="fa fa-file-excel-o"></i>&nbsp; {{'{{'}} trans('{{ $viewNamespace }}admin.{{ $modelLangFormat }}.actions.export') }}</a>
                    @endif
                    <a class="btn btn-primary btn-spinner btn-sm pull-right m-b-0" href="{{'{{'}} url('{{ $routePrefix }}admin/{{ $resource }}/create') }}" role="button"><i class="fa fa-plus"></i>&nbsp; {{'{{'}} trans('{{ $viewNamespace }}admin.{{ $modelLangFormat }}.actions.create') }}</a>
                </div>
                <div class="card-body" v-cloak>
                    <div class="card-block">
                        ...
                        <table class="table table-hover table-listing">
                            <thead>
                                <tr>
                                    ...
                                    <th is='sortable' class="text-center" :column="'{{ $col['name'] }}'">{{'{{'}} trans('{{ $viewNamespace }}admin.{{ $modelLangFormat }}.columns.{{ $col['name'] }}') }}</th>
                                    @else <th is='sortable' :column="'{{ $col['name'] }}'">{{'{{'}} trans('{{ $viewNamespace }}admin.{{ $modelLangFormat }}.columns.{{ $col['name'] }}') }}</th>
                                    @endif
                                    @endforeach
                                ...
                                <tr v-show="(clickedBulkItemsCount > 0) || isClickedAll">
                                    <td class="bg-bulk-info d-table-cell text-center" colspan="{{count($columns) + 2 }}">
                                        <span class="align-middle font-weight-light text-dark">@{{ trans('brackets/admin-ui::admin.listing.selected_items') }} {{'@{{'}} clickedBulkItemsCount }}. <a href="#" class="text-primary" @click="onBulkItemsClickedAll('/{{ $routePrefix }}admin/{{ $resource }}')" v-if="(clickedBulkItemsCount < pagination.state.total)"> <i class="fa" :class="bulkCheckingAllLoader ? 'fa-spinner' : ''"></i> @{{ trans('brackets/admin-ui::admin.listing.check_all_items') }} {{'@{{'}} pagination.state.total }}</a> <span class="text-primary">|</span> <a href="#" class="text-primary" @click="onBulkItemsClickedAllUncheck()">@{{ trans('brackets/admin-ui::admin.listing.uncheck_all_items') }}</a> </span>

                                        <span class="pull-right pr-2">
                                            <button class="btn btn-sm btn-danger pr-3 pl-3" @click="bulkDelete('/{{ $routePrefix }}admin/{{ $resource }}/bulk-destroy')">@{{ trans('brackets/admin-ui::admin.btn.delete') }}</button>
                                        </span>
                            ...
                            <tbody>
                                <tr v-for="(item, index) in collection" @if(!$withoutBulk):key="item.id" :class="bulkItems[item.id] ? 'bg-bulk' : ''" @endif>
                                    ...
                                    @elseif($col['name'] === 'published_at') <td class="text-center text-nowrap">
                                        ...
                                        <span v-if="item.published_at > now">
                                            <small>{{'{{'}} trans('{{ $viewNamespace }}admin.{{ $modelLangFormat }}.actions.will_be_published') }}</small><br />
                                            {{'@{{'}} item.published_at | datetime('DD.MM.YYYY, HH:mm') }}
                        ...
                        <div class="no-items-found" v-if="!collection.length > 0">
                            ...
                            <a class="btn btn-primary btn-spinner" href="{{'{{'}} url('{{ $routePrefix }}admin/{{ $resource }}/create') }}" role="button"><i class="fa fa-plus"></i>&nbsp; {{'{{'}} trans('{{ $viewNamespace }}admin.{{ $modelLangFormat }}.actions.create') }}</a>
                        </div>
                    </div>
                </div>
            </div>
